...
func TestPurposeExtension(t *testing.T) {
	def := addonExtension(t, addon.ExtKeyPurpose)

	tests := []struct {
		code cbc.Code
		name string
	}{
		{addon.PurposeNormal, "Normal"},
		{addon.PurposeComplementary, "Complementary"},
		{addon.PurposeAdjustment, "Adjustment"},
		{addon.PurposeGoodsReturn, "Goods Return"},
		{addon.PurposeCreditNote, "Credit Note"},
		{addon.PurposeDebitNote, "Debit Note"},
	}
...
func TestOperationTypeExtension(t *testing.T) {
	def := addonExtension(t, addon.ExtKeyOperationType)

	tests := []struct {
		code cbc.Code
		name string
	}{
		{addon.OperationInbound, "Inbound"},
		{addon.OperationOutbound, "Outbound"},
	}
	for _, tt := range tests {
		t.Run(tt.code.String(), func(t *testing.T) {
			cd := def.CodeDef(tt.code)
...
func TestCreditNoteTypeExtension(t *testing.T) {
	def := addonExtension(t, addon.ExtKeyCreditNoteType)
...
func TestDebitNoteTypeExtension(t *testing.T) {
	def := addonExtension(t, addon.ExtKeyDebitNoteType)
